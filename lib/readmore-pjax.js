...
module.exports = function (html, data) {
	...
	const execute = pageEnabled && !isExcluded ? 'yes' : 'no';
	...
	if (pageBody.length === 0) {
		if (debugEnabled) {
			console.log(colors.bold.white.bgYellow(' READMORE PLUGIN ') + ' not found article content by selector "' + pjaxSelector + '" for ' + data.path);
		}
		return html;
	}
	else if (pageBody.length > 1) {
		if (debugEnabled) {
			console.log(colors.bold.white.bgYellow(' READMORE PLUGIN ') + ' found multi article content by selector "' + pjaxSelector + '" for ' + data.path);
		}
		return html;
	}
	else if (pageBody.length === 1) {
		pageBody.attr('id', articleContentId);
